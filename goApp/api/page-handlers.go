...
func (server *Server) nairobiTourPage(ctx *gin.Context) {
	tmpl := templates.NairobiTourBody()
	if err := templates.NairobiTourPage(tmpl).Render(ctx, ctx.Writer); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("error generating template: %v", err) })
	}
}
...
func (server *Server) nairobiTourContact(ctx *gin.Context) {
	tmpl := templates.NairobiForm()
	if err := templates.ContactUsPage(tmpl).Render(ctx, ctx.Writer); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("error generating template: %v", err) })
	}
}
